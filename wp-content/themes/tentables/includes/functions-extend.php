...
function sandbox_insert_rewrite($rules) {
	$newrules = array();
	$newrules['(bands)/(.*)$'] = 'index.php?pagename=$recipes[1]&alpha=$recipes[2]';
...
// hook into the publishing of recipes, and create corresponding cheese taxonomy terms
add_action( 'save_post', 'tentables_update_cheese_taxonomy' );

function tentables_update_cheese_taxonomy( $post_id ) {

	// verify post is not a revision
	if ( !wp_is_post_revision( $post_id ) && get_post_status( $post_id ) == 'publish' ) {

		$current_post = get_post($post_id);
		// if we're dealing with a cheese
		if (get_post_type($current_post->ID) == 'deli-cheeses') {
			// get the cheese slug
			$current_post_slug = $current_post->post_name;
			// check if the corresponding cheese tax term exists (looking up the original term name)
			if (get_post_meta($post_id, '_recipe_store_term', true)) {
				$current_tax_term = get_term_by( 'slug', get_post_meta($post_id, '_recipe_store_term', true), 'cheese', 'OBJECT');
			} else {
				$current_tax_term = get_term_by( 'slug', $current_post_slug, 'cheese', 'OBJECT');
			}
			if ($current_tax_term) {
				// okay, the term exists, just update the term
				wp_update_term(
					$current_tax_term->term_id, 
					'cheese', 
					array(
				...
				wp_insert_term(
				  $current_post->post_title, // the term 
				  'cheese', // the taxonomy
				  array(
					'slug' => $current_post_slug
				  )
			...
			// save the term name so we can update the original later
			update_post_meta($post_id, '_recipe_store_term', $current_tax_term->slug);
		} else {
			// carry on
		}
...
// delete the shadow tax whenever a cheese is deleted
function tentables_sync_cheese_taxonomy($post_id) {
	$current_post = get_post($post_id);
	// if we're dealing with a cheese
	if (get_post_type($current_post->ID) == 'deli-cheeses') {
		// get the term associated with this post
		$current_tax_term = get_term_by( 'slug', $current_post->post_name, 'cheese', 'OBJECT');
		// delete the term
		wp_delete_term( $current_tax_term->term_id, 'cheese' );
	}
}
add_action('delete_post', 'tentables_sync_cheese_taxonomy', 10);
